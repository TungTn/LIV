<?php
/**
 * The template for displaying Restaurant Detail Page.
 * @package Miyatsu
...
<?php 
	...
	$terms = get_the_terms( $post->ID, 'restaurant' );
	$args = array(
						...
						<div class="restaurant-details">
									...
									<ul id="ratecv">
										...
										<li>
											...
											<span block-sc-pc>
												...
												of <?php echo count($total_post);?> Restaurants in <?php echo $primary_term->name;?>
											</span>
										</li>
									</ul>
								...
								<div class="address-details">
									<ul>
										<li class="position">
											...
											<span>
												<a href="#nearby" title="<?php echo get_field('address', $post->ID); ?>">
													<?php 
														if ( !empty(get_field('address', $post->ID)) ) { 
															echo wp_trim_words(apply_filters('address', get_field('address',  $post->ID )),'14');
														}else{
															echo "Hanoi Vietnam";
														}
													?>
										...
										<li class="web">
											...
											<span>
												<a href="http://<?php echo get_field('website', $post->ID); ?>" title="<?php echo get_field('website', $post->ID); ?>" target="_blank" rel="noopener noreferrer">
													<?php 
														if (!empty(get_field('website', $post->ID)) ) {
															$content = get_field('website', $post->ID);
							...
							<div class="restaurant-details-content-tab">
								<nav class="navigation" id="restaurantNav" block-sc-pc>
									<a class="navigation__link" href="#details">DETAILS</a>
								...
								<div class="page-section" id="details">
									...
									<div class="tab-details-content">
										<?php 
											if(!empty(get_field('custom_content',$post->ID,'option'))):
												$contents = get_field('custom_content',$post->ID,'option');
												foreach($contents as $content):
										?>
											<div class="box__total">
												<div class="box__total__title"><?php echo $content['title_section'];?></div>
												<div class="box__total__info">

													<?php echo wp_trim_words($content['content_section'],'20','<p data-content-short="'.mb_substr($content['content_section'], 0, 145).'" data-content="'.$content['content_section'].'"  class="comment_read_more" ><i>Read more...</i></p>');?>
												</div>
											</div>
											<?php endforeach;?>
										<?php endif;?>
